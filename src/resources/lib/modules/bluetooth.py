    ...
    class monitor:
        ...
        def PropertiesChanged(self, interface, changed, invalidated, path):
            try:
                ...
                for prop in changed:
                    if prop == 'Connected':
                        if self.oe.get_service_option('bluez', 'BLUETOOTH_ACTION_OPTION').replace('"', '') == '1': # play/pause
                            if changed[prop]:
                                ...
                                if xbmc.getCondVisibility("Player.Playing"):
                                    xbmc.Player().pause()
                        elif self.oe.get_service_option('bluez', 'BLUETOOTH_ACTION_OPTION').replace('"', '') == '2': # stop
                            if not changed[prop]:
                                if xbmc.getCondVisibility("Player.Playing"):
                                        xbmc.Player().stop()
