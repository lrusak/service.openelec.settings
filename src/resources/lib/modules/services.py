        ...
        try:
            ...
            if 'bluetooth' in self.oe.dictModules:
                if os.path.isfile(self.oe.dictModules['bluetooth'].BLUETOOTH_DAEMON):
                    ...
                    self.struct['bluez']['settings']['action_option']['values'] = ['none', 'playpause', 'stop']
                    self.struct['bluez']['settings']['action_option']['value'] = self.oe.get_service_option('bluez', 'BLUETOOTH_ACTION_OPTION').replace('"', '')
    ...
    def init_bluetooth(self, **kwargs):
        try:
            ...
            if not self.struct['bluez']['settings']['action_option']['value'] is None and not self.struct['bluez']['settings']['action_option']['value'] == 'none' and state == 1:
                options['BLUETOOTH_ACTION_OPTION'] = '"%s"' % self.struct['bluez']['settings']['action_option']['value']
